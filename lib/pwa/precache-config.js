...
// PWA-OFFLINE-UX-1. The single navigation-fallback target app/sw.ts serves
// when a NETWORK_ONLY document navigation fails offline — see
// docs/context/PWA_SW_DESIGN.md and the OFFLINE-UX-1 report for why this is
// a precached static page rather than caching the routes it stands in for.
...
function getBuildRevision() {
  ...
  try {
    ...
    for (const route of [...OFFLINE_SAFE_ROUTES, ...STATIC_SHELL_ROUTES, OFFLINE_FALLBACK_ROUTE]) {
...
/**
 * The seven calculator documents, their two navigational shells, and the
 * offline fallback page, revisioned by build/commit so a deploy that changes
 * their HTML replaces the precached copy instead of leaving it stale
 * indefinitely.
 */
function getDocumentEntries(revision) {
  return [...OFFLINE_SAFE_ROUTES, ...STATIC_SHELL_ROUTES, OFFLINE_FALLBACK_ROUTE].map((url) => ({
    url,
    revision,
  }));
}
